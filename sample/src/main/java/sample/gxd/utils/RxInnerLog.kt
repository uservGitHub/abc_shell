 ...
 */
class RxInnerLog private constructor() {
    /**
     * 调用方式：
     * 1. 重置（总控设置、分控设置）
     * 2. 加入桩信息
     ...
     */
    companion object {
        val INSTANCE: RxInnerLog by lazy {
            RxInnerLog()
        }
    }
     ...
     */
    fun resetRxModel(allEndAction: ((RxInnerLog) -> Unit)? = null):Boolean {
        //运行中，无法设置
        if (checkRunning()){
            return false
        }
     ...
     * 分控设置，本次有效，最后要重置
     */
    fun reset(endAction:((RxInnerLog)->Unit)? = null, hasAutoPillNext:Boolean = false):Boolean{
        //运行中，无法设置
        if (checkRunning()){
            return false
        }
    ...
    private var endActionFromRxModel:((RxInnerLog)->Unit)? = null
    private var endAction:((RxInnerLog)->Unit)? = null
